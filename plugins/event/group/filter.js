      ...
      try {
         if (groupSet.filter && !isAdmin && isBotAdmin && !m.fromMe) {
            ...
            if (body && (new RegExp('\\b' + toxic.join('\\b|\\b') + '\\b')).test(body.toLowerCase())) {
               ...
               if (warning > 4) return client.reply(m.chat, Func.texted('bold', `Sudah kelewat batas ini orang, utiwi tendang...`), m).then(() => {
               ...
               return client.reply(m.chat, `*PERINGATIN* \n\nEnte dapet PERINGATAN : [ ${warning} / 5 ]\n\Kalo ente toxic lagi, bakalan ane tendang dari grup aowkwok.`, m).then(() => client.sendMessage(m.chat, {
                  delete: {
                     remoteJid: m.chat,
                     fromMe: isBotAdmin ? false : true,
                     id: m.key.id,
